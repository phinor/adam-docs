# Third Party Integration {#h-bh1uvqo6yoh}

## API Integration with ADAM {#h-3kpxm1oe3xmj}

ADAM provides a RESTful JSON API which technical users can make use of. For more information, please see the [dedicated section discussing the API](api-access-to-adam.md#h-kvj4kzzehoy5).

## Clever Integration with ADAM {#h-3if6fssvb1ei}

@todo

## DevMan Integration with ADAM {#h-5xvmhdlmue6k}

The DevMan module has several API endpoints that are customised for DevMan’s specific needs.

### Create an API Token for DevMan {#h-gnspozkmgdkr}

Follow the instructions for [creating a new API Token](api-access-to-adam.md#h-m0b4zvwuxgjc).
...
### Configuring DevMan {#h-a8r6j4exrbtv}
...
## Ed-Admin Integration with ADAM {#h-4ox6s2req8pq}
...
### Configuring the Integration {#h-jcaqqlxcfcok}
...
-   **Ed-Admin Grade Definitions:** While there is a field here that can be manually edited, please rather see the section below on [Setting up Grade Definitions](#h-c3ecp38j1gv6).

Once done, click on **Save Settings**.

### Setting Up Grade Definitions {#h-c3ecp38j1gv6}
...
### Running a Manual Synchronisation {#h-a1c0bwhundsr}
...
If you do not see these buttons, and rather only see the option to “Perform a synchronisation check for Grades”, then you must complete this step first. [Instructions are above](#h-c3ecp38j1gv6).
...
### Changes in the Change Log {#h-go5neuv80yw7}
...
## Google Integration with ADAM {#h-esi2zcckv7u5}

@todo

## Moodle Integration with ADAM {#h-r6e811tzgrsr}

Please see the separate section on [Moodle Integration](moodle-integration.md#h-36ei31r).

## Papyrus Integration with ADAM {#h-7q72rmr9e6qz}
...
### Create an API Key for Papyrus {#h-23ldp6bete2s}

Follow the instructions for [creating a new API Key](api-access-to-adam.md#h-m0b4zvwuxgjc).
...
### Create Data Query Secrets for Papyrus {#h-9jv8vb14hnb6}

Two DataQuery Secrets [need to be created](api-access-to-adam.md#h-ucmmird25yvj): one for staff, and one for pupils. The secrets must all be associated with the API Key that you created in the first step.
...
### Configuring Papyrus {#h-6ph0c9wg5vm8}
...
### Create Data Query Secrets for Wonde {#h-2krruocse8ui}

You must first create an API Token (described [above](#h-1p96bzhgou5d)) before you can proceed.
...
### Information to Share with Wonde {#h-qdoychx4bvy8}
...
## SOCS Integration with ADAM {#h-ykpl3cyesqjv}
...
### SOCS Export {#h-e8hvaz23yrv9}

To get the exports working, you will need to do the following:

1.  In [Site Settings](changing-site-settings.md#h-3j2qqm3), under the **Third Party Integration** tab, look for the “SOCS” section.
...
### Pupil Identification to SOCS {#h-qbgc15r4f3v0}
...
## SportCap Integration with ADAM {#h-o0yi5xa79sek}
...
-   Two [DataQuery Secrets](api-access-to-adam.md#h-ucmmird25yvj) both linked to the API Token above, one for Pupils and the other for Staff. The fields required are:
