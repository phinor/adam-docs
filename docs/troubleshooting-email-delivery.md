...
It occasionally happens that a recipient will not receive an email message that has been sent by ADAM. This is most notable when parents, for example, are waiting for a [password reset email](parent-and-pupil-portal.md#forgotten-passwords) that never arrives.
