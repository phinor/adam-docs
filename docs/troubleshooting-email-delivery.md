# Troubleshooting Email Delivery {#h-91ugk3ocgive}

It occasionally happens that a recipient will not recieve an email message that has been sent by ADAM. This is most notable when parents, for example, are waiting for a [password reset email](parent-and-pupil-portal.md#h-eu2jzab8dpvd) that never arrives.
...
## 1) Is the email address captured correctly in ADAM? {#h-i4besut8vz4p}
...
### Beware identity fraud! {#h-xcju7jrw3wrw}
...
## 2) Has the email been delivered to their “spam” or “junk” folder? {#h-edjdlbdstxpq}
...
### Reducing spam mail detection {#h-kks08lodjfml}

If you find that mail sent from ADAM is commonly detected as spam or junk mail, there are some proactive steps that your school’s IT department can take to reduce this from happening. Please see the separate section on how to [mitigate against spam problems](messaging-centre.md#h-gv5eyps3xomu).

## 3) Can the IT department shed any light? {#h-b3zcmzqjoidl}
