# Messaging Centre {#h-o6jbiu0gh9e}

## Sending Messages from the Messaging Centre {#h-aqrj3oa9484d}

There are a number of entry points into the messaging centre.

### Messaging current pupils or parents {#h-rgplaq38belw}
...
-   **Message:** You can type in your message here. Please see the section below on [Composing your Message](#h-sn7696umzdje).
...
### Merge Codes {#h-n2m4ml74zyhp}

Below the email window is a list of merge codes that can be used in your message. These are the same fields that you can call on when you draw up a [scratch list](scratch-lists.md#h-bg6zukpa7qwi) of pupils.
...
#### A warning about context {#h-bxjkw82syk2q}
...
### Duplicated Messages within Batches {#h-r37hr13xdjf1}
...
## Privileges Required for using the Messaging Centre {#h-7hnkzdvk2imv}

Before a user can send messages from the messaging centre, they need a few privileges to be set first.

### Step 1: {#h-ogdm0cwxakdw}

The first privilege is a general one that is applied to the whole privilege group. They require, at a minimum, “Can make use of the messaginge centre (*messaging\_centre*)”. This is found on the **Messaging** tab, and under the heading **E-mail**. For more information on changing staff privileges, please see [Security Administration for Staff](security-administration-for-staff.md#h-3ls5o66) elsewhere in this documentation.

### Step 2: {#h-ab7l5wji511e}
...
If you wish to set privileges for other messaging modules, you will need to enable these modules in the [Site Settings](logging-on-to-adam.md#h-6vhh2g12hnrg). A link to the correct place inthe site settings is provided at the top of the page. Note that while it is possible to set privileges for enabled modules, the module must have the correct configuration details supplied in the Site Settings in order for it to be capable of delivering your messages.
...
## Managing Messaging Centre Batches {#h-m0b84s3v3oxa}
...
### Delaying a Batch {#h-8f1nvmmccd73}
...
## Sending SMSs from the Messaging Centre {#h-40rn5a8own7a}

Please see [SMS Service](sms-services.md#h-1pgrrkc) elsewhere in this manual.

## The Messaging Centre and Email Spam {#h-gv5eyps3xomu}
...
### Sender Framework Policy (SPF) Configuration {#h-f3noiacd48nx}
...
### Domain Keys Identified Mail (DKIM) Signing {#h-sflv3il9fmlm}
...
## Messaging a Pupil’s Teachers {#h-lp7ixvsnv7z}
