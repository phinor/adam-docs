...
ADAM uses three third party service providers to send SMS messages from the [Messaging Centre](#h-78so5mmskbgv). These are:
...
![](assets/screenshots/sms-services/sms-services-01.png)*Check to see which teachers have the* *[privileges to send SMS messages](#h-9bzq0453i38z)* *and that they are aware of the school’s policy for communicating with parents.*
...
A statistics report is available to see how many cell numbers on your system are enabled to receive SMS messages from the [messaging centre](#h-78so5mmskbgv). To see the report, navigate to “Administration” and under the “Database administration” heading, click on “View SMS statistics”.
...
Sending SMS messages is done through the messaging centre. Please see more information about [Messaging Centre](#h-78so5mmskbgv).
