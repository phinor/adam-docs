...
It is possible to choose which privilege groups are sent alerts. For example, you may wish for some groups to have Family Alerts sent only to parents and some groups sent also to pupils. This is done in the [Pupil Login Privileges](security-administration-for-families-and-pupils.md#h-jikjh4kunfbk) section. You will want to enable to **Family Alerts** privilege for pupils and/or parents.

## Setting Alerts for Login Groups {#h-cw64jwqaqfd6}
...
Note that if there is a “hyphen” in the **Sent to…** column, you will need to [change the privileges for that group](#h-oosetdrl3h8) before alerts will be sent.
...
## Customising Alert Content {#h-af7f3tfgpyba}

Currently the assessments and absentee alerts do not provide for any customisation in content.

Records and points, however, will follow the same settings which define whether the records and points categories are shown on the parent portal. More can be found in the information regarding [adding and editing Records and Points](records-and-points-administration.md#h-3lnsfyslep9m) later in this manual.

## Customising the Covering Email {#h-llf1762y5cxd}

You will need to customise the **Family Alerts** template in the [email templates](email-message-templates.md#h-5rkfadj40kta).
