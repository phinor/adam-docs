# Reporting Period Administration {#h-1a346fx}
...
## Adding a new Reporting Period {#h-3u2rp3q}
...
## Editing a Reporting Period {#h-u5xim02o9rs5}
...
### Report Template Options {#h-l6qh65ir0rze}
...
-   The **Report Template** field determines which template will be used with the grade that has been selected. Note that ADAM will only show enabled templates here (unless a selected template has since been disabled, then it, too, will show). See the section on [manging reporting templates](report-publishing.md#h-25xaf5x26yvq) for more information.
...
### Reporting Screen Options {#h-8cc6u4ifzipy}
...
-   It is also possible to have ADAM display the **Learning Outcome Levels**. LO Levels can be drawn automatically from the markbook. Since we have now done away with Outcomes Based Education, many schools prefer to think of Learning Outcomes as “topic areas” instead. Have a look at the section on [managing Learning Outcomes](learning-outcomes-and-assessment-standards.md#h-deo5jvne1x4g). *Note that your reporting template must specifically support Outcome Levels for them to display.*
...
### Calculation Information and Options {#h-vp15nnb6dya9}
...
-   The **Year-to-Date Calculation** determines how ADAM calculates the pupils’ year-to-date results (or, at the end of the year, effectively their promotion result). This dropdown shows a list of [Aggregated Results Calculations](aggregated-results.md#h-pczqwhx0flp6). You can choose one here. If you need to set one up first, you can do so and return later to edit this reporting period and select the calculation.
...
-   Other uses a manually entered calculation to calculate the aggregate. This is [explained in some detail below](#h-nh3ofrfj26iq).

### Other Settings and Advanced Options {#h-gb1lwv9752ic}
...
-   If you want to enable the [pupil goals](goal-setting.md#h-8s2wn2s66a9n) module of ADAM, you can set that option to “Yes”.
...
## Reporting Period Time Frames {#h-38czs75}
...
### Adding Time Frames {#h-aphgqr1vl8sh}
...
### Clearing Time Frames {#h-ylmmr9hvj2}
...
### Multiple Time Frames {#h-j5fufwb4oh4z}
...
### Adding an Overlapping Time Frame {#h-dt37wkoaryfr}
...
### Reporting Period Actions {#h-3jrzg5mf3ttu}
...
#### Markbook Embargo {#h-3dqhwltxp90t}
...
#### Report Comment Entry {#h-gtue3r44sv0n}
...
#### Report Mark Entry {#h-9v313fqe480m}
...
#### Report Level Entry {#h-5cg8n5mfwzwf}
...
#### Behavioural Indicator Entry {#h-gh11f6x9z2i1}
...
#### Report Comment Editing {#h-6pgfobwwicrr}
...
#### Report Mark Editing {#h-xut4vz8wastd}
...
#### Report Level Editing {#h-705dz7eopxw0}
...
#### Behavioural Indicator Editing {#h-s4d6r8lxe3cg}
...
#### Capture Goals (for pupils) {#h-lgdu4w5bdipn}

If set, this setting determines when pupils are able to set and make changes to their goals. See elsewhere in the manual for more information on [Goal Setting](goal-setting.md#h-8s2wn2s66a9n).

## Custom Aggregate Calculations {#h-nh3ofrfj26iq}

This is discussed in its [own separate section](aggregate-calculations.md#h-n3thyo3fac7f).
